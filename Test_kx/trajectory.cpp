...
void Trajectory::draw_trajectory() {
    ...
    ui->widget->addGraph();
    ui->widget->graph (2) -> setPen (QPen (Qt :: red)); // Цвет кисти для рисования кривой 2 красный
    ...
    ui->widget->graph(3)->addData(X[222][0],X[223][0]); //перемещение аппарата 3

    ui->widget->addGraph();
    ui->widget->graph (4) -> setPen (QPen (Qt :: red)); // Цвет кисти для рисования кривой 4 красный
    ui->widget->graph(4)->addData(X[322][0],X[323][0]); //перемещение аппарата 4

    ui->widget->addGraph();
    ui->widget->graph (5) -> setPen (QPen (Qt :: red)); // Цвет кисти для рисования кривой 5 красный
    ui->widget->graph(5)->addData(X[422][0],X[423][0]); //перемещение аппарата 5
